...
// NickTool 昵称工具
type NickTool struct {
}

// NewNickTool 实例化昵称工具
func NewNickTool() *NickTool {
	return &NickTool{}
}

func (n *NickTool) getLastName() []string {
	return []string{
...
func (n *NickTool) getFirstName() []string {
	return []string{
...
// GetRand 获取随机昵称
func (n *NickTool) GetRand() string {
	rand.Seed(time.Now().UnixNano())
	n1 := n.getFirstName()
	n2 := n.getLastName()
	a := len(n.getFirstName())
	b := len(n.getLastName())
	var first string
	for i := 0; i <= rand.Intn(1); i++ {
		first = fmt.Sprint(n1[rand.Intn(a-1)])
	}
	return fmt.Sprintf("%s%s", fmt.Sprint(n2[rand.Intn(b-1)]), first)
}
